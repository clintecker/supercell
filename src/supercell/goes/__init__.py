...
IMAGE_PATTERN = r"https://{host}(?P<image_path>[" \
                r"a-zA-Z0-9/:\-_.]+)',\s+".format(host=re.escape(IMAGE_HOST))
IMAGE_COMPPAT = re.compile(IMAGE_PATTERN)

FILE_PATTERN = r"/GOES(?P<sat_num>\d{2})" \
               r"/ABI/SECTOR/" \
...
IMAGE_COMPOSE = "/GOES{sat_num}/ABI/SECTOR/{sector}/{band}/{t_year}" \
...
def image_url_from_data(image_data: Dict) -> str:
    ...
    return urlunparse(["https", IMAGE_HOST, image_path, None, None,
                       None])
...
def full_day_images(sat: str,
                    sector: str,
                    band: str,
                    size: int = 600,
                    anchor_datetime: Union[datetime, None] = None) -> List:
    ...
    pattern_string = r"(?P<file_name>(?P<year>{yearone}|{yeartwo})(?P<day_num>{day_num}|{prev_day_num})(?P<time>[" \
    ...
    if index_response.status_code == requests.codes.ok:
        ...
        matches = {
            ...
            for i_filename, _, i_daynum, i_time
            in pattern.findall(index_response.text)
            if int(i_daynum) == day_num or (int(i_daynum) == prev_day_num and int(i_time) > now_time)
        }
