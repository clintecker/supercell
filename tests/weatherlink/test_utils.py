...
class SupercellWeatherlinkUtilitiesTestSuite(unittest.TestCase):
    def test_crc16(self):
        ...
        crc = supercell.weatherlink.utils.crc16(
            data=bs, crc=0, table=supercell.weatherlink.utils.CRC16_TABLE
        )
        self.assertEqual(64217, crc)

    def test_connect(self):
        with mock.patch("socket.socket") as mock_sock_class:
            mock_sock = supercell.weatherlink.utils.connect(
                host="23.23.23.24", port=9999
            )
    ...
    def test_request(self):
        ...
        mock_sock.recv.return_value = bytes(
            [supercell.weatherlink.utils.ACKNOWLEDGED_RESPONSE_CODE,]
        )
        supercell.weatherlink.utils.request(sock=mock_sock, body=b"1234567")
        self.assertTrue(mock_sock.sendall.called_with(b"1234567"))
        self.assertTrue(
            mock_sock.recv.called_with(supercell.weatherlink.utils.RESPONSE_CODE_SIZE)
        )

        mock_sock.recv.return_value = bytes(
            [supercell.weatherlink.utils.NACK_RESPONSE_CODE,]
        )
        with self.assertRaises(supercell.weatherlink.exceptions.NotAcknowledged):
            supercell.weatherlink.utils.request(sock=mock_sock, body=b"1234567")

        mock_sock.recv.return_value = bytes(
            [supercell.weatherlink.utils.BAD_CRC_RESPONSE_CODE,]
        )
    ...
    def test_receive_data(self):
        ...
        data = supercell.weatherlink.utils.receive_data(sock=mock_sock)
        self.assertEqual(b"\xaa\xff", data)
        self.assertTrue(
    ...
    def test_make_datetime(self):
        t = [
            (
                BitStream(bytes([0x28, 0xD6])),
                1239,
                datetime.datetime(2020, 6, 22, 12, 39, 0),
                None,
            ),
